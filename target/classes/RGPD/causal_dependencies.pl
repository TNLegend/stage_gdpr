:- multifile notAvailable/2.
:- dynamic  notAvailable/2.
:- use_module(library(tabling)).
:- table wasDerivedFromP/2.
:- table isPersonalP/2.

% === CAUSAL DEPENDENCIES ===
wasTriggeredBy(P1,P2,TU) :- used(P1,D,_R1,TU) , wasGeneratedBy(D,P2,_R2,_TG).
wasDerivedFrom(D1,D2,R1,TG) :-
    wasGeneratedBy(D1,P,_R,TG),
    used(P,D2,R1,_TU),
    D1 \= D2.
...
wasDerivedFromP(D1,D2) :- wasDerivedFrom(D1,D2,_,_).
wasDerivedFromP(D1,D2) :- wasDerivedFrom(D1,M,_,_), wasDerivedFromP(M,D2).
